...
LOCAL_MODELS = [
    ...
    {"id":"home-assistant-sft(moe)","name":"Home Assistant (moe)","hf_repo":"LiquidAI/LFM2.5-8B-A1B-GGUF","hf_file":"LFM2.5-8B-A1B-Q8_0.gguf","size_label":"9.01 GB","score_label":"99%"},
...
def _start_llama_server_bg(model: dict) -> None:
    ...
    if model_path:
        cmd = [base_cmd, "--model", model_path, "--port", "8080", "--ctx-size", "2048", "--threads", "4"]
    else:
        cmd = [base_cmd, "--hf-repo", model["hf_repo"], "--hf-file", model["hf_file"], "--port", "8080", "--ctx-size", "2048", "--threads", "4"]
